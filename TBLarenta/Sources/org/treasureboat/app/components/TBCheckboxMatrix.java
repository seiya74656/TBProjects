 ...
 * @binding tableOtherTagString - どんな文字列もオープニングタグの一部にさせます。これは "checked" 又は "selected" のような独立型の属性をタグの一部にします。
...
public class TBCheckboxMatrix extends ERXNonSynchronizingComponent {
	...
	public void setSelections(NSArray<String> aFormValuesArray) {
		if (aFormValuesArray != null && !disabled()) {
			...
			// this can be an EOEnterpriseObject but also a list of Enums, so use Object here
			...
			if (relationshipName() != null && relationshipName().length() > 0 && relationshipOwner() != null
					&& aSelectionsArray.firstObject() instanceof EOEnterpriseObject) {
	...
	public String otherTagStringForCheckBox() {
		...
		return (isDisabled ? "disabled" : TBFConstants.EMPTY_STRING) + (isDisabled && isChecked ? " " : TBFConstants.EMPTY_STRING)
				+ (isChecked ? "checked" : TBFConstants.EMPTY_STRING);
	}
